...
from DiffuGene.VAEembed.vae import SNPVQVAE, VQVAEConfig, eval_batch_categorical
from DiffuGene.utils import ensure_dir_exists
from DiffuGene.utils.file_utils import read_bim_file
from DiffuGene.VAEembed.prepare_vqvae_data import write_raw_to_h5_fast, h5_is_complete, build_snp_info
...
def load_model(model_path: str, device: torch.device) -> SNPVQVAE:
    """Load a trained SNPVQVAE model from a checkpoint and move it to device."""
...
def evaluate_chromosome(model: SNPVQVAE,
                        chr_h5_dir: str,
                        device: torch.device,
                        results_dir: str,
                        chr_no: int,
                        ...
                        bim_path: Optional[str] = None) -> Dict[str, any]:
    """
    ...
    with torch.no_grad():
        for idx_h5, h5p in enumerate(h5_files, start=1):
            ...
            for s in range(0, N, batch_size):
                ...
                xb = xb.to(device)
                logits3, z_q_seq, commit_loss, indices_list, stats_list, mask_tokens = model(xb)
                # metrics
                m = eval_batch_categorical(model, xb, device=device, verbose=False)
                total_acc.append(m['acc'])
                total_mse.append(m['mse'])
                probs = torch.softmax(logits3, dim=1)
                pred = probs.argmax(dim=1).cpu()
                cm_sum += compute_confusion_counts(pred, xb.cpu())
                # AFs
                x_hat = (probs * torch.tensor([0.0, 1.0, 2.0], device=probs.device).view(1, 3, 1)).sum(dim=1).cpu().numpy()
                af_true_all.append((xb.cpu().numpy().mean(axis=0) / 2.0).astype(np.float32))
                ...
                # correctness per SNP
                sum_correct += (pred.cpu().numpy() == xb.cpu().numpy()).sum(axis=0)
...
def main():
    ...
    p.add_argument('--models-dir', required=True, help='Directory containing per-chromosome model checkpoints')
    p.add_argument('--model-pattern', default='vqvae_chr{chr}.pt', help='Filename pattern for per-chromosome models')
    ...
    print(f"[VAL] Starting validation for chromosomes={chromosomes}")
    ...
    for chr_no in chromosomes:
        ...
        if not os.path.exists(model_path):
            ...
            continue
        print(f"[VAL] Loading model for chr{chr_no}: {model_path}")
        model = load_model(model_path, device)
        chr_dir = os.path.join(args.val_h5_dir, f"chr{chr_no}")
        out_dir = os.path.join(args.results_dir, f"chr{chr_no}")
        ensure_dir_exists(out_dir)
        m = evaluate_chromosome(model, chr_dir, device, out_dir, chr_no,
...
# VQVAE_DIR=/n/home03/ahmadazim/WORKING/genGen/UKBVQVAE/
# python ./validate_vqvae.py \
...
#   --val-h5-dir ${VQVAE_DIR}/genomic_data/val_h5_cache/ \
#   --models-dir ${VQVAE_DIR}/models/vqvae/ \
#   --model-pattern vqvae_chr{chr}.pt \
#   --results-dir ${VQVAE_DIR}/val_res256/ \
#   --chromosomes all \
#   --device cuda \
#   --batch-size 512 \
#   --ld-block-dir ${VQVAE_DIR}/../UKB6PC/genomic_data/haploblocks/ 