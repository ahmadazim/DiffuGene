...
class RecodedData(pd.DataFrame):
    ...
    def get_variant_names(self):
        """Returns the variant names (column names from the 7th column onward)."""
        return list(self.columns[6:])

    def get_bp_coordinates(self):
        """Parses and returns the base-pair coordinates from variant names."""
        coords = []
        for col in self.columns[6:]:
            try:
                bp = int(col.split(":")[1])
            except (IndexError, ValueError):
                raise ValueError(f"Invalid variant column name format: {col}")
            coords.append(bp)
