...
def generate(args):
    ...
    # Initialize scheduler (DDIM, epsilon prediction, no sample clipping)
    ...
    # # No sigma_hat scaling in generation (disabled)
    ...
    while samples_generated < total_samples:
        ...
        # Initialize from Gaussian noise and denoise with DDIM
            ...
            for step_i, t in enumerate(tqdm(scheduler.timesteps, desc=f"Denoising batch {batch_index + 1}")):
                ...
                if DEBUG and (step_i % 10 == 0):
                    ...
                    if not ok:
                        ...
                        break
        # No sigma scaling post-denoise
        if DEBUG:
            _tensor_stats("latents(final)", latents, log_fn=logger.info)
