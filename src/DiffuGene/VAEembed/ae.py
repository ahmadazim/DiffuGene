...
# NEW HELPERS FOR STAGE 2 REGULARIZATION
def latent_tv_loss(z: torch.Tensor) -> torch.Tensor:
    """
    Total-variation-like smoothness penalty on 2D latent.
    z: (B, C, H, W)
...
def decode_expected_dosage(model: nn.Module, z: torch.Tensor) -> torch.Tensor:
    """
    Decode latent z with AE and return expected dosage E[X | logits].
    Assumes model.decode(z) -> logits of shape (B,3,L).
    """
    logits = model.decode(z)  # (B,3,L)
...
def encode_from_embed(
    ...
    """
    # 1D downsample
    h1d, _ = ae._downsample_1d(h, mask)
    # 1D -> 2D
    h2d = h1d.permute(0, 2, 1).unsqueeze(1)  # (B,1,M1D,2^K1)
    h2d = ae.proj2d(h2d)
    # 2D down blocks
    for down_block in ae.down2d_blocks:
        h2d = down_block(h2d)
    z = h2d
    return z
...
class GenotypeAutoencoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # Variational heads (μ, logσ²) and bookkeeping
        self.mu_head = nn.Conv2d(self.latent_channels, self.latent_channels, kernel_size=1, bias=True)
        self.logvar_head = nn.Conv2d(self.latent_channels, self.latent_channels, kernel_size=1, bias=True)
    ...
    def decode_with_latent_mask(
        ...
        *,
        ...
        self._last_active_fraction = float(c_prime) / float(self.latent_channels)
    ...
    def _encode_to_latent(self, x: torch.Tensor) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        """Shared encoder used by deterministic and variational forwards."""
        h, mask = self._prepare_input(x)
        h1d, mask1d = self._downsample_1d(h, mask)
        h2d = h1d.permute(0, 2, 1).unsqueeze(1)
        h2d = self.proj2d(h2d)
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Encode → apply VAE heads → sample via reparameterization → decode.
        Returns (logits, sampled_latent).
        """
        self._last_latent_mask = None
        self._last_active_fraction = 1.0
        z_det, mask = self._encode_to_latent(x)
        mu = self.mu_head(z_det)
        logvar = self.logvar_head(z_det)
        z_sample = self._reparameterize(mu, logvar)
    ...
    def loss_function(
        ...
        *,
        ...
        if self._last_mu is not None and self._last_logvar is not None and used_beta > 0.0:
            kl_tensor = -0.5 * (
                1 + self._last_logvar - self._last_mu.pow(2) - self._last_logvar.exp()
            )
            C_tot = float(self.latent_channels)
            H = self._last_mu.shape[2]
            W = self._last_mu.shape[3]
            if self._last_latent_mask is not None:
                mask = self._last_latent_mask.to(kl_tensor.device)
                active_channels = mask.sum()
                kl_tensor = kl_tensor * mask.view(1, mask.shape[1], 1, 1)
            else:
                active_channels = torch.tensor(C_tot, device=kl_tensor.device)
            active_voxels = active_channels * H * W + 1e-8
            kl_per_sample = kl_tensor.sum(dim=(1, 2, 3)) / active_voxels
...
#             sum_kl += float(metrics.get("kl", torch.tensor(0.0)).item()) * bs
#             count += bs
...
#             f"maf={mean_maf:.5f} | ld={mean_ld:.5f} | kl={mean_kl:.5f} | lr={float(lr_value):.6f}"
#         )
...
def train_vae(
    ...
    *,
    ...
    robust_lambda: float = 1e-3,
    stable_lambda: float = 1e-3,
    latent_noise_std: float = 0.05,
    embed_noise_std: float = 0.05,
    stage2_start_frac: float = 0.75,
    ...
    Stage 1 (first 75% of epochs): standard VAE + masking + MAF/LD losses.
    Stage 2 (last 25% of epochs): add latent regularizers to the loss:
        - TV smoothness in latent space
        - robust subspace penalty in data space
        - stable subspace penalty in latent space

    At the end of each epoch (regardless of stage), we evaluate and log:
        - TV, robust, and stable penalties on the training set
        - TV, robust, and stable penalties on the validation set (if provided)
    ...
    if beta_kl is not None:
        model.beta_kl = float(beta_kl)

    # ------------------------------------------------------------------
    # Helper: validation MSE (expected-dosage space)
    # ------------------------------------------------------------------
    @torch.no_grad()
    def _eval_validation_mse(model_eval: nn.Module, loader: torch.utils.data.DataLoader) -> float:
        if loader is None:
    ...
    # ------------------------------------------------------------------
    # Helper: encode from embedding with gradient (for stage-2 training)
    # Mirrors encode_from_embed but WITHOUT @torch.no_grad().
    # ------------------------------------------------------------------
    def _encode_from_embed_grad(
        ae: GenotypeAutoencoder,
        h: torch.Tensor,
        mask: Optional[torch.Tensor],
    ) -> torch.Tensor:
    ...
    # ------------------------------------------------------------------
    # Helper: evaluate latent penalties (TV / robust / stable) on a loader
    # Uses encode_from_embed + decode_expected_dosage under no_grad.
    # ------------------------------------------------------------------
    @torch.no_grad()
    def _eval_latent_penalties(
        model_eval: GenotypeAutoencoder,
        ...
        *,
        ...
        if loader is None:
            return {"tv": float("nan"), "robust": float("nan"), "stable": float("nan"), "stable_norm": float("nan")}
        was_training = model_eval.training
        model_eval.eval()
        ...
        for xb in loader:
            ...
            # Embed once
            h, mask = model_eval._prepare_input(xb)              # (B, embed_dim, L_t), (B,L_t) or None

            # Deterministic latent (no VAE sampling)
            z = encode_from_embed(model_eval, h, mask)           # (B, C, H, W)

            # 1) Patch smoothness (TV in latent space)
            tv_val = latent_tv_loss(z)

            # 2) Robust subspace: MSE(X, D(z + δz))
            if latent_noise_std_eval > 0.0:
                noise_lat = latent_noise_std_eval * torch.randn_like(z)
                z_pert = z + noise_lat
            else:
                z_pert = z
            x_hat_pert = decode_expected_dosage(model_eval, z_pert)   # (B,L)
            robust_val = F.mse_loss(x_hat_pert, xb.float())

            # 3) Stable subspace: MSE(z, z_tilde), noise in embedding h
            if embed_noise_std_eval > 0.0:
                noise_emb = embed_noise_std_eval * torch.randn_like(h)
                h_noisy = h + noise_emb
            else:
                h_noisy = h
            z_noisy = encode_from_embed(model_eval, h_noisy, mask)
    ...
    # ------------------------------------------------------------------
    # Stage-2 schedule: last 25% of epochs
    # ------------------------------------------------------------------
    stage2_start_frac = float(stage2_start_frac)
    stage2_start_frac = max(0.0, min(1.0, stage2_start_frac))
    stage2_start_epoch = int(math.floor(stage2_start_frac * float(num_epochs))) + 1
    stage2_start_epoch = max(1, min(int(num_epochs), stage2_start_epoch))
    logger.info(
    ...
    for epoch in range(1, int(num_epochs) + 1):
        ...
        # Whether to include stage-2 penalties in the *training* loss this epoch
        stage2_active = (
        ...
        for xb in dataloader:
            ...
            with torch.autocast(
                ...
                # Standard VAE forward + masked decoding
                logits_full, z_sample = model(xb)
                if hasattr(model, "decode_with_latent_mask") and getattr(
                    model, "enable_structured_latent", False
                ):
                    logits = model.decode_with_latent_mask(z_sample, enable_masking=True)
                else:
                    logits = logits_full
                ...
                if stage2_active:
                    ...
                    # Deterministic latent (no sampling) for penalties
                    z_det, _ = model._encode_to_latent(xb)   # (B,C,H,W)

                    # (a) TV smoothness in latent space
                    if tv_lambda > 0.0:
                        tv_val = latent_tv_loss(z_det)
                        stage2_loss = stage2_loss + tv_lambda * tv_val

                    # (b) Robust subspace: MSE(X, D(z_det + δz))
                    if robust_lambda > 0.0 and latent_noise_std > 0.0:
                        noise_lat = latent_noise_std * torch.randn_like(z_det)
                        z_pert = z_det + noise_lat
                        x_hat_pert = decode_expected_dosage(model, z_pert)
                        robust_val = F.mse_loss(x_hat_pert, xb.float())
                        stage2_loss = stage2_loss + robust_lambda * robust_val

                    # (c) Stable subspace: MSE(z_clean, z_noisy) for embedding noise
                    if stable_lambda > 0.0 and embed_noise_std > 0.0:
                        h_emb, mask_emb = model._prepare_input(xb)
                        z_clean = _encode_from_embed_grad(model, h_emb, mask_emb)
                        noise_emb = embed_noise_std * torch.randn_like(h_emb)
                        h_noisy = h_emb + noise_emb
                        z_noisy = _encode_from_embed_grad(model, h_noisy, mask_emb)
        ...
        logger.info(
            ...
            f"lr={float(lr_value):.6f} | stage2_active={stage2_active}"
        )

        # Validation + early stopping (based on val MSE)
        if val_dataloader is not None:
            val_mse = _eval_validation_mse(model, val_dataloader)
            logger.info(f"Epoch {epoch}/{num_epochs}: val_mse={val_mse:.6f}")
        else:
        ...
        # --------------------------------------------------------------
        # At the end of each epoch: evaluate latent penalties on
        #   - training set
        #   - validation set (if provided)
        # --------------------------------------------------------------
        train_pen = _eval_latent_penalties(
            model,
            dataloader,
        ...
        if val_dataloader is not None:
            val_pen = _eval_latent_penalties(
                model,
                val_dataloader,
                latent_noise_std_eval=latent_noise_std,
                embed_noise_std_eval=embed_noise_std,
            ...
            val_pen = {"tv": float("nan"), "robust": float("nan"), "stable": float("nan"), "stable_norm": float("nan")}
