...
from diffusers import UNet2DModel, DDPMScheduler, DDIMScheduler  # type: ignore
import re
...
from DiffuGene.VAEembed.vae import GenotypeVAE  # type: ignore
from DiffuGene.VAEembed.ae import VAEConfig, local_ld_penalty
from DiffuGene.utils.file_utils import read_bim_file


def load_vae_from_checkpoint(vae_ckpt_path: str, device: torch.device) -> Tuple[GenotypeVAE, VAEConfig]:
    """
    Load a trained VAE checkpoint that includes a 'config' and a state dict.
    Uses 'last_state_dict' (NOT best). Falls back to 'model_state' or raw dict.
    """
    # ckpt = torch.load('/n/home03/ahmadazim/WORKING/genGen/UKBVQVAE/models/AE/ae_chr22.pt', map_location="cpu")
    # cfg_dict = ckpt["config"]
    # cfg = VAEConfig(**cfg_dict)

    # vae_model = GenotypeVAE(
    ...
    #     beta_kl=1e-5,
    # )
    # ckpt_vae = torch.load('/n/home03/ahmadazim/WORKING/genGen/UKBVQVAE/models/VAE/vae_chr22_warmup5_beta1e-5.pt', map_location="cpu")
    # final_state_dict = ckpt_vae["last_state_dict"]
    # vae_model.load_state_dict(final_state_dict)
    ...
    payload = torch.load(vae_ckpt_path, map_location="cpu")
    state = payload.get("last_state_dict")

    m = re.search(r"beta([0-9eE\+\-\.]+)", os.path.basename(vae_ckpt_path))
    beta_kl = float(m.group(1)) if m else 0
    if beta_kl == 0:
        raise ValueError(f"Invalid beta_kl: {beta_kl}")
    print(f"Loading VAE with beta_kl={beta_kl}")
    ...
    missing = vae.load_state_dict(state, strict=False)
    if getattr(missing, "unexpected_keys", None):
        raise RuntimeError(f"VAE state mismatch (unexpected keys): {missing.unexpected_keys}")
    vae.to(device).eval()
...
class LatentUNET2D_16(nn.Module):
    """
    Unconditional UNet for 16x16 latents with latent_channels channels.
    """
    def __init__(self, input_channels: int = latent_channels, output_channels: int = latent_channels, base_channels: int = base_channels) -> None:
        super().__init__()
        self.input_proj = nn.Conv2d(input_channels, base_channels, kernel_size=3, padding=1)
        # 16 -> 8 -> 4 -> 2 -> 1 (4 downsamples)
...
def encode_h5_to_memmap(vae: GenotypeVAE,
    ...
    """
    Encode a single H5 batch file (X dataset) to a memmap of latents (N, latent_channels, 16, 16).
    Returns (memmap_out, shape).
    """
    with h5py.File(h5_batch_path, 'r') as f:
        X = f['X']
        N, L = int(X.shape[0]), int(X.shape[1])
        C, M = latent_channels, int(vae.M2D)
        shape = (N, C, M, M)
...
def main() -> None:
    ...
    parser.add_argument("--vae-model-path", type=str, required=True, help="Path to trained VAE checkpoint (.pt)")
    ...
    # 1) Load VAE
    vae, cfg = load_vae_from_checkpoint(args.vae_model_path, device=device)
    ...
    def _encode_h5s_to_memmap(vae: GenotypeVAE,
                              h5_paths: list[str],
                              memmap_out: str,
                              device: torch.device,
                              encode_batch_size: int = 128) -> tuple[str, tuple[int, int, int, int]]:
        ...
        for p in h5_paths:
            with h5py.File(p, 'r') as f:
                ...
                if first_shape_L is None:
                    first_shape_L = int(X.shape[1])
        C, M = latent_channels, int(vae.M2D)
        shape = (total_N, C, M, M)
        mm = create_memmap(memmap_out, shape)
        ...
        for p in h5_paths:
            ...
            with h5py.File(p, 'r') as f:
                ...
                for s in range(0, N, encode_batch_size):
                    ...
                    xb = xb.to(device, non_blocking=(device.type == 'cuda'))
                    with torch.no_grad():
                        logits, z, mu_z, logvar_z, mask = vae(xb)
                        z_cpu = z.detach().to('cpu').float().numpy()
    ...
    # 5) Build UNet for 16x16 with latent_channels channels
    print(f"Building UNet for 16x16 with latent_channels channels")
    model = LatentUNET2D_16(input_channels=latent_channels, output_channels=latent_channels, base_channels=base_channels).to(device)
    ...
    for epoch in range(1, int(args.epochs) + 1):
        ...
        for x_cpu in train_loader:
            ...
            pred = model(x_noisy, t)  # v prediction
            # v_target = sqrt(alpha_bar)*eps - sqrt(1-alpha_bar)*x0
            alpha_bar = scheduler.alphas_cumprod[t].view(B, 1, 1, 1).to(device)  # [B,1,1,1]
            s = torch.sqrt(alpha_bar)
            r = torch.sqrt(1.0 - alpha_bar)
            v_true = s * noise - r * x0
            # v loss only on active channels
            diff_sq = (pred - v_true).pow(2) * ch_mask
            denom = float(B * C_act * H * W)
            loss_v = diff_sq.sum() / max(1.0, denom)
            # Predict clean latents from v_pred
            x0_pred = s * x_noisy - r * pred  # normalized latent
            # Denormalize to original latent space
            z_pred_full = x0_pred * sd_dev + mu_dev
            z_clean = x0 * sd_dev + mu_dev
            # Pass-through inactive channels
            z_pred = z_pred_full * ch_mask + z_clean * (1.0 - ch_mask)
            # Decode to pixel-space logits using frozen VAE decoder
            vae.eval()
            with torch.no_grad():
                logits_clean = vae.decode_full(z_clean)  # [B,3,L]
                y_target = logits_clean.argmax(dim=1)  # Long [B,L]
            logits_pred = vae.decode_full(z_pred)  # allow grad wrt z_pred (vae params frozen)
            # CE loss in pixel space
            loss_ce = F.cross_entropy(logits_pred, y_target, reduction="mean")
            # Optional simple penalties (as in train.py)
            ...
            ld_pen = local_ld_penalty(x_tgt, x_hat, window=128) if x_tgt.size(1) >= 128 else torch.tensor(0.0, device=x0.device)
            loss = loss_v + 1e-2 * loss_ce + 1e-3 * maf_pen + 1e-3 * ld_pen
            loss.backward()
            ...
            count += B
            del x0, x_noisy, noise, pred, t
            # if device.type == "cuda":
            #     torch.cuda.empty_cache()
        print(f"[Epoch {epoch}/{args.epochs}] loss={total_loss / max(1, count):.6f}")
    ...
    orig_lat = np.asarray(mm[:n_use])  # (n_use, latent_channels, 16, 16)
    del mm
    ...
    eval_model = LatentUNET2D_16(input_channels=latent_channels, output_channels=latent_channels, base_channels=base_channels).to(device)
    if USE_EMA_FOR_EVAL:
    ...
    mu_np = mu.view(1, latent_channels, 1, 1).cpu().numpy()
    sd_np = sd.view(1, latent_channels, 1, 1).cpu().numpy()
    gen_lat = gen_norm_cpu * sd_np + mu_np
    del gen_norm, gen_norm_cpu
    # if device.type == "cuda":
    #     torch.cuda.empty_cache()

    # For inactive channels, set to mean; then restrict diagnostics to active channels
    C_act = int(max(1, min(int(args.active_channels), latent_channels)))
    if C_act < latent_channels:
    ...
    dims = rng.choice(D, size=min(D, 16**2 * latent_channels), replace=False)
    ...
    vae.eval()
    with torch.no_grad():
        logits_o = vae.decode_full(z_orig_t)  # [B,3,L]
        logits_g = vae.decode_full(z_gen_t)
