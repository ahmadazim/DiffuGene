...
from DiffuGene.VAEembed.ae2 import GenotypeAutoencoder, VAEConfig, local_ld_penalty
...
class TimestepEmbedder(nn.Module):
    """
    Sinusoidal + MLP time embedding, similar in spirit to DiT / DDPM++.
    """
    ...
    def forward(self, t: torch.Tensor) -> torch.Tensor:
        """
        t: [B] long or float, assumed in [0, T-1] or [0,1].
        ...
        freqs = torch.exp(
            -math.log(10000.0) * torch.arange(0, half_dim, device=t.device).float() / float(half_dim)
        )  # [half_dim]
        args = t[:, None] * freqs[None, :]  # [B, half_dim]
        emb = torch.cat([torch.cos(args), torch.sin(args)], dim=-1)  # [B, time_dim]
...
class DiTBlock(nn.Module):
    ...
    A single DiT-style block:
      - LayerNorm
      - Time-conditioned FiLM (scale, shift)
    ...
    def __init__(self, hidden_size: int, num_heads: int, mlp_ratio: float = 4.0):
        ...
        # Time modulation (FiLM-style): produces [scale, shift] of size hidden_size
        self.time_mod = nn.Linear(hidden_size, 2 * hidden_size)
    ...
    def forward(self, x: torch.Tensor, t_emb: torch.Tensor) -> torch.Tensor:
        ...
        """
        # FiLM params
        scale, shift = self.time_mod(t_emb).chunk(2, dim=-1)  # each [B, hidden_size]

        # Self-attention block
        h = self.norm1(x)
        h = h * (1.0 + scale[:, None, :]) + shift[:, None, :]
        h_attn, _ = self.attn(h, h, h)  # [B, N, hidden_size]
        x = x + h_attn

        # MLP block
        h2 = self.norm2(x)
        h2 = h2 * (1.0 + scale[:, None, :]) + shift[:, None, :]
        x = x + self.mlp(h2)
...
class LatentDiT2D(nn.Module):
    """
    Diffusion Transformer for HxW latents with C channels.
    - Flattens spatial dims into N = H*W tokens
    - Projects C -> hidden_size
    - Applies a stack of DiTBlocks with time-conditioning
    - Projects hidden_size -> C and reshapes back to (B, C, H, W)

    This is a drop-in replacement for LatentUNET2D_16: same (x, t) -> eps_pred interface.
    """
    def __init__(
        self,
        ...
        # Token embedding: treat each (H,W) position as a token of dim C
        self.token_proj = nn.Linear(input_channels, hidden_size)

        # Learned positional embeddings over N = H*W tokens
        self.num_tokens = sample_size * sample_size
        self.pos_embed = nn.Parameter(torch.zeros(1, self.num_tokens, hidden_size))

        # Time embedding (timestep → hidden_size)
        self.time_embedder = TimestepEmbedder(time_dim=time_embed_dim, hidden_dim=hidden_size)

        # Stack of DiT blocks
        self.blocks = nn.ModuleList(
            [DiTBlock(hidden_size=hidden_size, num_heads=num_heads, mlp_ratio=mlp_ratio) for _ in range(depth)]
        )

        # Final LayerNorm + projection back to channels
        self.norm_out = nn.LayerNorm(hidden_size)
        self.out_proj = nn.Linear(hidden_size, output_channels)
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        """
        x: [B, C, H, W]
        t: [B] long timesteps (0..num_train_timesteps-1)
        Returns: eps_pred [B, C, H, W]
        """
        B, C, H, W = x.shape
        assert H == self.sample_size and W == self.sample_size, \
            f"LatentDiT2D expects sample_size={self.sample_size}, got H={H}, W={W}"

        # Flatten spatial dims: (B, C, H, W) -> (B, N, C)
        x_tokens = x.view(B, C, H * W).permute(0, 2, 1)  # [B, N, C]
        h = self.token_proj(x_tokens)                    # [B, N, hidden]

        # Add positional embeddings
        h = h + self.pos_embed[:, :h.size(1), :]

        # Time embedding
        # map integer timesteps [0, T-1] to a normalized float range (0..1) for embedding
        t_norm = t.float() / float(self.max_time_steps - 1)
        t_emb = self.time_embedder(t_norm)  # [B, hidden]

        # DiT blocks
        for blk in self.blocks:
            h = blk(h, t_emb)  # [B, N, hidden]

        # Final norm and projection back to channels
        h = self.norm_out(h)                # [B, N, hidden]
        eps_tokens = self.out_proj(h)       # [B, N, C_out]

        # Reshape back to (B, C_out, H, W)
        eps = eps_tokens.permute(0, 2, 1).contiguous().view(B, self.output_channels, H, W)
        return eps
...
class LatentUNET2D_16(nn.Module):
    ...
    def __init__(self, input_channels: int = 64, output_channels: int = 64, base_channels: int = 256, sample_size: int = 16) -> None:
        ...
        self.unet = UNet2DModel(
            ...
            layers_per_block=2,
            block_out_channels=[base_channels, 2 * base_channels, 3 * base_channels, 4 * base_channels],
...
def main() -> None:
    ...
    def _encode_h5s_to_memmap(ae: GenotypeAutoencoder,
        ...
        # if C != 64 or M != 16:
        #     raise RuntimeError(f"Expected AE latent 64x16x16, got C={C}, M={M}")
        shape = (total_N, C, M, M)
        mm = create_memmap(memmap_out, shape)
    ...
    # Pre-training diagnostic: per-channel spatial variance (up to 1000 samples)
    try:
    ...
    # Pre-training diagnostic: per-channel spatial mean (up to 1000 samples)
    try:
    ...
    # # 3) Compute per-channel normalization over the memmap
    ...
    # # 5) Build UNet for HxW latents with 64 channels (use sample_size=32)
    # print(f"Building UNet with sample_size={unet_sample_size} for latent shape C={C}, H={H}, W={W}")
    # if args.use_XL_UNET:
    #     model = LatentUNET2D_16_XL(input_channels=64, output_channels=64, base_channels=base_channels_XL, sample_size=unet_sample_size).to(device)
    # else:
    #     model = LatentUNET2D_16(input_channels=64, output_channels=64, base_channels=base_channels, sample_size=unet_sample_size).to(device)
    # model = model.to(memory_format=torch.channels_last)

    
    # 5) DDPM scheduler and optimizer
    ...
    try:
        ...
        print(f"Set prediction type to epsilon")
    except Exception:
        pass
    scheduler.alphas_cumprod = scheduler.alphas_cumprod.to(device)
    scheduler.betas = scheduler.betas.to(device)

    # 6) Build DiT for HxW latents
    print(f"Building DiT with sample_size={unet_sample_size} for latent shape C={C}, H={H}, W={W}")
    model = LatentDiT2D(
        input_channels=C,
        output_channels=C,
        ...
        mlp_ratio=4.0,
        time_embed_dim=256,
        max_time_steps=scheduler.config.num_train_timesteps,
    ).to(device)
    model = model.to(memory_format=torch.channels_last)
    ...
    for epoch in range(1, int(args.epochs) + 1):
        ...
        for x_cpu in train_loader:
            ...
            # Masked noisy latent and masked noise (Eq. (2) style)
            x_noisy_masked = x_noisy * ch_mask
            noise_masked = noise * ch_mask

            # Predict noise on masked latents only
            pred = model(x_noisy_masked, t)
            pred_masked = pred * ch_mask

            # SNR-unweighted MSE on masked channels
            diff_sq = (pred_masked - noise_masked).pow(2)
            loss_eps = diff_sq.mean()

            # Recover clean latents on masked channels in the same coordinate as x0
            alpha_bar = scheduler.alphas_cumprod[t].view(B, 1, 1, 1).to(device)  # [B,1,1,1]
            x0_pred_masked = (
                x_noisy_masked - torch.sqrt(1.0 - alpha_bar) * pred_masked
            ) / torch.sqrt(alpha_bar + 1e-12)
            ...
            # Start with epsilon MSE as the core loss
            loss = loss_eps
            ...
            if args.use_decoded_losses:
                ...
                loss = loss_eps + 1e-2 * loss_ce + 1e-3 * maf_pen + 1e-3 * ld_pen
    ...
    @torch.no_grad()
    def _sample_uncond_ddpm(model: nn.Module, num_samples: int, num_steps: int, device: torch.device) -> torch.Tensor:
        sched = DDPMScheduler(
            num_train_timesteps=1000,
            beta_schedule="linear",
            beta_start=1e-4,
    ...
    # if args.use_XL_UNET:
    #     eval_model = LatentUNET2D_16_XL(input_channels=64, output_channels=64, base_channels=base_channels_XL, sample_size=unet_sample_size).to(device)
    # else:
    #     eval_model = LatentUNET2D_16(input_channels=64, output_channels=64, base_channels=base_channels, sample_size=unet_sample_size).to(device)
    # if USE_EMA_FOR_EVAL:
    #     eval_model.load_state_dict(ema.module.state_dict())
    # else:
    #     eval_model.load_state_dict(model.state_dict())

    # Choose evaluation weights (EMA or last) for DiT
    eval_model = LatentDiT2D(
        input_channels=C,
        output_channels=C,
        sample_size=unet_sample_size,
        hidden_size=512,
    ...
    gen_norm = _sample_uncond_ddpm(eval_model, num_samples=n_use, num_steps=1000, device=device)
