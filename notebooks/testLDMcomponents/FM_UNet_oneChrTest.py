...
def main() -> None:
    ...
    for epoch in range(1, int(args.epochs) + 1):
        ...
        for x_cpu in train_loader:
            ...
            # ---- Flow matching: sample τ and base noise z ----
            # τ ~ Uniform(0,1), but avoid exact 0 or 1 for numerical reasons
            tau = torch.rand(B, device=device)
            tau = tau.clamp(1e-4, 1.0 - 1e-4)            # [B]

            z = torch.randn_like(x0)                     # base noise ~ N(0, I)

            # Interpolant x_τ = (1 - τ) x0 + τ z
            tau_view = tau.view(B, 1, 1, 1)
            x_tau = (1.0 - tau_view) * x0 + tau_view * z

            # Forward flow velocity target: v*(x_τ, τ) = z - x0
            v_true = (z - x0)

            # ---- DC-AE channel masking (same idea as before) ----
            idx = torch.randint(
                0,
            ...
            x_tau_masked = x_tau * ch_mask
            v_true_masked = v_true * ch_mask

            # ---- Condition UNet on τ (discretized to 0..999) ----
            # We reuse the existing timestep embedding machinery by mapping τ ∈ [0,1]
            # into integer "timesteps" 0..(num_train_timesteps-1).
            T = scheduler.config.num_train_timesteps  # e.g. 1000
            t_cond = (tau * (T - 1)).long()           # [B]

            v_pred = model(x_tau_masked, t_cond)      # [B, C, H, W]
            v_pred_masked = v_pred * ch_mask

            # Flow-matching MSE on velocity
            loss_fm = (v_pred_masked - v_true_masked).pow(2).mean()

            # For now, we use *only* the flow-matching loss
            loss = loss_fm

            # (Optional) decoded losses would need to be re-derived for FM;
            # easiest is to disable them for now:
            if args.use_decoded_losses:
                raise NotImplementedError(
                    "Decoded losses not yet adapted for flow matching. "
                    "Run without --use-decoded-losses for now."
                )
            ...
            del x0, x_tau, z, v_true, v_pred, tau, t_cond, x_tau_masked, v_true_masked, v_pred_masked
    ...
    @torch.no_grad()
    def _sample_uncond_flow_matching(
        model: nn.Module,
        num_samples: int,
        num_steps: int,
        device: torch.device,
        ...
        """
        Simple ODE sampler for flow matching:
        - Start from x_1 ~ N(0, I)
        - Integrate d x_tau / d tau = v_theta(x_tau, tau) from tau=1 -> 0
        """
        model.eval()

        # Start at tau = 1 with Gaussian noise
        x = torch.randn(num_samples, C, H, W, device=device).to(memory_format=torch.channels_last)

        # Uniform grid in tau from 1 -> 0
        taus = torch.linspace(1.0, 0.0, steps=num_steps + 1, device=device)  # [num_steps+1]
        dt = -1.0 / num_steps  # negative (since we go 1 -> 0)

        for i in range(num_steps):
            tau_i = taus[i]   # current τ
            tau_vec = tau_i * torch.ones(num_samples, device=device)

            # Map tau into timestep embedding index 0..T_embed-1
            t_cond = (tau_vec * (T_embed - 1)).long()

            v_pred = model(x, t_cond)   # [B, C, H, W]

            # Reverse-time Euler step: x_{tau+dt} = x_tau + dt * v
            x = x + dt * v_pred
    ...
    # gen_norm = _sample_uncond_flow_matching(eval_model, num_samples=n_use, num_steps=1000, device=device)
    gen_norm = _sample_uncond_flow_matching(
        eval_model,
        num_samples=n_use,
        num_steps=1000,
        device=device,
        ...
        H=H,
        W=W,
        T_embed=scheduler.config.num_train_timesteps,  # typically 1000
    )
