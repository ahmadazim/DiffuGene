#!/usr/bin/env python
"""
Generate samples from a trained DDPM UNet using DDPM sampling (from pure Gaussian noise).
...
- Starts from x_T ~ N(0, I) and runs DDPM for num_steps down to t=0.
...
def ddpm_generate(
    ...
    """
    Generate latents from pure Gaussian noise, using DDPM.
    ...
    # Setup DDPM scheduler consistent with training (betas schedule must match!)
    ...
    # ddpm.set_timesteps(num_inference_steps=num_steps, device=device)  # descending
    ...
    while remaining > 0:
        ...
        for t in ddpm.timesteps:
            ...
            step = ddpm.step(model_output=model_out, timestep=t, sample=x)
            x = step.prev_sample
...
def main():
    ...
    p.add_argument("--num-steps", type=int, default=50, help="Sampling steps")
    p.add_argument("--eta", type=float, default=0.0, help="Ignored for DDPM")
    ...
    # Generate
    latents, cov_used = ddpm_generate(
        model=model,
